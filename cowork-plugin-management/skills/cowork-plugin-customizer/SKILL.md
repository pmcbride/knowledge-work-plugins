...
description: >
  Customize a Cursor plugin for a specific organization's tools and workflows.
...
> **Finding the plugin**: To find the plugin's source files, search the Cursor local plugins directory:
> ```bash
> find ~/.cursor/plugins/local -type d -name "*<plugin-name>*"
> find ~/.cursor/plugins/cache -type d -name "*<plugin-name>*"
> ```
> Read the plugin files to understand its structure before making changes. If you cannot find the plugin directory, ask the user to navigate to the plugin folder and share the path.
...
> **Legacy `commands/` directories**: Some plugins include a `commands/` directory. Cursor presents these alongside skills as a single "Skills" concept, so treat `commands/*.md` files the same way you would `skills/*/SKILL.md` files when customizing.
...
## Delivering the Customized Plugin

After all customizations are applied, tell the user what was changed and how to use the updated plugin:

```markdown
Customization complete. Here's what was updated:

- [Change 1]
- [Change 2]

Your plugin files are at: <path-to-plugin-directory>

The plugin is already active in Cursor — changes take effect immediately.
If you installed it from the marketplace, re-copy the directory to ~/.cursor/plugins/local/<plugin-name>/ to apply updates.
```

> **Naming**: Do not rename the plugin or its files during customization — only replace placeholder values and update content.
